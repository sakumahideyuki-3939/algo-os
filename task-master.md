...
- [ ] サロン向け事業（salon-project）
  ...
  - [x] 顧客管理Phase 1-A完了（16ファイル/2,588行）＋本番動作確認OK
  - [x] 顧客管理Phase 1-B写真カルテ完了
  - [x] 顧客管理Phase 2-1〜2-4完了（写真描き込み・ビフォーアフター・予約ブロック・自動DM）
  - [x] 経理-1A完了（ダッシュボード+レジ締め+簡易PL）＋本番確認OK
  - [x] 経理-1B+2完了（分析+CSV+メール）
  - [x] 出勤-1A+1B完了（打刻+シフト+有給+残業+経理連動）＋本番確認OK
  - [x] cron設定（daily_report 21:00 / attendance_alert 22:00）
  - [x] DBマイグレーション3回＋文字化け修正（latin1→utf8mb4）＋MySQL予約語修正
  - [ ] 🔴 DBパスワード変更（SSHコマンドに平文漏れ→要変更）
  ...
  - [ ] 柳さん追加ヒアリング（次回会った時）
    ...
    - HPBをやめるとしたら「これだけは困る」機能
    - 石井さん1人のとき、柳さんはどう状況把握しているか
    - 同等機能が月1-2万円なら乗り換え検討するか
  - [ ] Phase 3 / 経理-3 / 出勤-2
  - [ ] IBCAブログ記事1本目
  - [ ] 全体設計図に⑩（スタッフ管理）追加・更新
  - [ ] 全体設計図にターゲット3層（1A/1B/2）反映

### 💡 検討中・将来
- [ ] LINE連携サブスクリプション販売アプリ（美容サロン向け）

## ✅ 完了タスク
- [x] salon-project 顧客管理・経理・出勤管理3モジュール完走（2026/03/26）30項目実施。顧客管理Phase1-2完了（カルテ+写真描き込み+ビフォーアフター+予約ブロック+自動DM）、経理1A-2完了（ダッシュボード+レジ締め+PL+分析+CSV+メール）、出勤1A-1B完了（打刻+シフト+有給+残業+経理連動）、cron2件設定、文字化け/予約語/リダイレクトループ修正
